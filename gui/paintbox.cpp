...
G_DEFINE_TYPE(GxSD1PaintBox, gx_paint_box, GTK_TYPE_BOX)

static void gx_paint_box_class_init (GxSD1PaintBoxClass *klass)
{
...
static void set_paint_func(GxSD1PaintBox *paint_box, const gchar *paint_func)
{
	gchar *spf;
...
static void gx_paint_box_style_set(GtkWidget *widget, GtkStyle  *previous_style)
{
	GxSD1PaintBox *paint_box = GX_SD1_PAINT_BOX(widget);
	set_paint_func(paint_box, paint_box->paint_func);
}


static void gx_paint_box_init (GxSD1PaintBox *paint_box)
{
...
static void gx_paint_box_destroy(GtkObject *object)
{
	GxSD1PaintBox *paint_box = GX_SD1_PAINT_BOX(object);
	...
	GObject *object, guint prop_id, GValue *value, GParamSpec *pspec)
{
	switch (prop_id) {
	case PROP_PAINT_FUNC:
		g_value_set_string(value, GX_SD1_PAINT_BOX(object)->paint_func);
		break;
	default:
		G_OBJECT_WARN_INVALID_PROPERTY_ID(object, prop_id, pspec);
		break;
...
static void pedal_expose(GtkWidget *wi, GdkEventExpose *ev)
{
	...
	cairo_t *cr;
	GxSD1PaintBox *paintbox = GX_SD1_PAINT_BOX(wi);
