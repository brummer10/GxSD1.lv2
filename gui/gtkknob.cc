...
#define GTK_SD1_KNOB_GET_PRIVATE(obj) (G_TYPE_INSTANCE_GET_PRIVATE((obj), GTK_TYPE_SD1_KNOB, GtkSD1KnobPrivate))

G_DEFINE_TYPE(GtkSD1Knob, gtk_knob, GTK_TYPE_RANGE);

typedef struct _GtkSD1KnobPrivate GtkSD1KnobPrivate;

struct _GtkSD1KnobPrivate
{
    int knob_x;
    int knob_y;
    int knob_step;
...
static void knob_expose(GtkWidget *widget, int knob_x, int knob_y, GdkEventExpose *event, int arc_offset)
{
    /** check resize **/
    int grow;
    GtkSD1KnobPrivate *priv = GTK_SD1_KNOB_GET_PRIVATE(widget);
    ...
    if (priv->model != 3) {
        ...
        GtkSD1Knob *knob = GTK_SD1_KNOB(widget);
...
{
    g_assert(GTK_IS_SD1_KNOB(widget));
    GtkSD1Knob *knob = GTK_SD1_KNOB(widget);
    GtkSD1KnobPrivate *priv = GTK_SD1_KNOB_GET_PRIVATE(knob);
...
{
    GtkSD1Knob *knob = GTK_SD1_KNOB(object);
...
/****************************************************************
 ** init the GtkSD1KnobClass
 */

static void gtk_knob_class_init (GtkSD1KnobClass *klass)
{
...
//--------- connect the events with funktions
    ...
    g_type_class_add_private(obj_class, sizeof (GtkSD1KnobPrivate));
}
...
static void gtk_knob_init (GtkSD1Knob *knob)
{
...
{
    GtkWidget *widget = GTK_WIDGET( g_object_new (GTK_TYPE_SD1_KNOB, NULL ));
